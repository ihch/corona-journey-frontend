...
const Pulldown: React.VFC<PulldownProps> = ({
  items,
  ...
  return (
    <Listbox value={selected} onChange={setSelected}>
      <div className="relative w-full">
        <Listbox.Button className="w-full text-gray-700 text-xl font-bold px-10 py-2 border-2 rounded-lg hover:border-teal-300">
          {selected?.name || placeholder} {!selected ? "👇" : "👍"}
        </Listbox.Button>
        <Listbox.Options className="w-full max-h-80 overflow-y-auto absolute bottom-0 left-0 translate-y-full border-2 rounded-lg bg-white">
          {items.map((item) => (
            <Listbox.Option
